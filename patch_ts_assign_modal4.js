...
smContent = smContent.replace(
  /const \[selectedSeatId, setSelectedSeatId\] = useState<string \| null>\(null\);/g,
...
smContent = smContent.replace(
  /const handleSeatClick = \(seat: Seat\) => {[\s\S]*?setSelectedElement\(\{ id: seat\.id, type: 'seat' \}\);[\s\S]*?};/g,
...
console.log('Regex replace fixed errors');
